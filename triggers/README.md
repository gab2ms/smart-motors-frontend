...
- `categorias_lancamento` (tabela) — CRUD das categorias do modal de lançamento (`lancamentos.categoria`). Colunas: `nome` TEXT UNIQUE, `tipo` TEXT CHECK in (`entrada`,`saida`,`ambos`), `ativa` BOOLEAN, `ordem` INTEGER (reordenação custom, futuro), `cor` TEXT (visual, futuro), `criado_em`. RLS ativa + policy `acesso_total`. Sem `TABLE_MAP` entry — mappers genéricos cobrem. Seed inicial: 33 categorias (23 detectadas em `lancamentos` + 10 do select `#lan-categoria` sem lançamento); `tipo` auto-detectado pelos lançamentos, exceto `Estorno` forçado a `ambos` (estorno pode ser entrada ou saída — amostra pequena só tinha saídas). Sub-commit A da feature de CRUD de categorias; o sub-commit B integra no modal.
  - **TODO (sessão futura):** Contas a Pagar/Receber ainda usam o select `#cp-categoria` hardcoded e podem divergir do select de Lançamento (`#lan-categoria`). Decidir depois: unificar com `categorias_lancamento` ou criar uma tabela `categorias_contas` separada.
  - **TODO (sessão futura) — categoria "Bloqueado":** hoje é uma categoria de lançamento (`tipo = 'ambos'`), mas conceitualmente representa um *estado da conta bancária* (dinheiro em análise/quarentena), não uma despesa/receita. Considerar: feature dedicada de "saldo bloqueado" por conta bancária, ou flag `em_analise BOOLEAN` em `lancamentos`; migrar os lançamentos existentes pra a nova estrutura. Mantida como categoria por enquanto pra não quebrar o histórico.
