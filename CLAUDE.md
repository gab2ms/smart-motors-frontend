...
- **Estado:** ✅ implementado e validado no localhost com sessão real (boot limpo, modal renderiza, deriva barras, upload/download/remove no bucket OK, RLS lê as 4 parcelas). **Em validação local (não commitado/pushed).** PDF do boleto ainda não anexado (`boleto_path` null) — anexar pela UI (2 cliques).
